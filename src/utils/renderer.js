...
// Listen for background transparency updates from main process shortcuts
ipcRenderer.on('apply-transparency', (event, alpha) => {
    const colors = theme.get(theme.current);
    theme.applyBackgrounds(colors.background, alpha);

    // Update CustomizeView if it's currently rendered so the slider matches
    const appEl = cheatingDaddy.element();
    if (appEl && appEl.currentView === 'customize') {
        const customizeView = appEl.shadowRoot.querySelector('customize-view');
        if (customizeView) {
            customizeView.backgroundTransparency = alpha;
